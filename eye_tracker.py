...
while(True):
    ...
    for rect in rects:
        ...
        result = print_eye_pos(img, eyeball_pos_left, eyeball_pos_right)
        # for (x, y) in shape[36:48]:
        #     cv2.circle(img, (x, y), 2, (255, 0, 0), -1)
        #cprint(result)
        if(result == 1):
            resetTime = False
        if(result == -1 and resetTime == False):
            #print("here")
            start = time.perf_counter()
            resetTime = True
        #add in the sound into the next if statement
        end = time.perf_counter()
        timeDiff = end-start
        #print(resetTime,  timeDiff)
        if(resetTime == True and timeDiff >= 7):
            playsound('alarmbeep.wav')
